...
/* Walk every item in a payload (internal pointer items are skipped). */
void
event_payload_foreach(struct event_payload *ep, event_payload_foreach_cb cb,
    void *data)
{
	struct event_payload_item	*epi;
	struct event_payload_value	 epv;
	...
	RB_FOREACH(epi, event_payload_tree, &ep->items) {
		...
		switch (epi->type) {
		case EVENT_PAYLOAD_STRING:
			epv.string = epi->string;
			break;
		case EVENT_PAYLOAD_TIME:
			epv.time = epi->time;
			break;
		case EVENT_PAYLOAD_INT:
			epv.number = epi->number;
			break;
		case EVENT_PAYLOAD_UINT:
			epv.unsigned_number = epi->unsigned_number;
			break;
		case EVENT_PAYLOAD_CLIENT:
			epv.client = epi->client;
			break;
		case EVENT_PAYLOAD_SESSION:
			epv.session = epi->session;
			break;
		case EVENT_PAYLOAD_WINDOW:
			epv.window = epi->window;
			break;
		case EVENT_PAYLOAD_PANE:
			epv.pane = epi->pane;
			break;
