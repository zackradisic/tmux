...
# grants, and A's view talks to that copy (an initiator's calls into the
# remote are always allowed).
#
...
# A -> B (initiator -> remote) is always allowed, so A's view reaches B's
# own copy with no grant.
