...
/* Deliver a success completion and wake the drain machinery. */
static void
plugin_async_done(uint64_t token, int64_t v0, int64_t v1, const u_char *data,
    size_t len)
{
	pgh_async_complete(token, 0, v0, v1, data, len);
	plugin_schedule_drain();
}

/* Deliver an error completion (err = PGH_ERR_*, msg = the message). */
static void
plugin_async_error(uint64_t token, int err, const char *msg)
{
	pgh_async_complete(token, err, 0, 0, (const u_char *)msg,
	    msg != NULL ? strlen(msg) : 0);
	plugin_schedule_drain();
}

/*
 * Job complete: deliver exit status and output. The output bytes are
 * handed straight from the job's evbuffer (the host copies before
 * returning); truncated at PLUGIN_JOB_OUTPUT_MAX.
 */
static void
plugin_job_complete(struct job *job)
{
	...
	if (size > PLUGIN_JOB_OUTPUT_MAX)
		size = PLUGIN_JOB_OUTPUT_MAX;

	pjob->completed = 1;
	plugin_async_done(pjob->token, retcode, signalled,
	    EVBUFFER_DATA(event->input), size);
}
...
plugin_job_free(void *data)
{
	...
	if (!pjob->completed)
		plugin_async_error(pjob->token, PGH_ERR_CANCELLED,
		    "job cancelled");
	free(pjob);
}
 ...
 * Completion: v0 = exit status (or signal number), v1 = signalled,
...
plugin_run_command_done(__unused struct cmdq_item *item, void *data)
{
	...
	plugin_async_done(prc->token, 0, 0, NULL, 0);
	free(prc);
...
plugin_vtable_run_command(const char *cmdstr, uint64_t token)
{
	...
	if (pr->status == CMD_PARSE_ERROR) {
		plugin_async_error(token, PGH_ERR_BAD_REQUEST, pr->error);
		free(pr->error);
		return (0);
	}
...
plugin_timer_fire(__unused int fd, __unused short events, void *data)
{
	...
	plugin_async_done(pt->token, 0, 0, NULL, 0);
	free(pt);
}
