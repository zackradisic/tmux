...
fn register_imports(linker: &mut Linker<StoreData>) -> wasmtime::Result<()> {
    ...
    linker.func_wrap(m, im::FS_READ_LINES, |mut c: Caller<'_, StoreData>, path_ptr: i32, path_len: i32, offset: i64, needles_ptr: i32, needles_len: i32, head: i32, max_line: i32, out_ptr: i32, out_cap: i32| -> i64 {
        ret_i64(with_mem(&mut c, |mem| {
            dispatch::fs_read_lines_async(mem, path_ptr, path_len, offset, needles_ptr, needles_len, head, max_line, out_ptr, out_cap)
        }))
    })?;

    linker.func_wrap(m, im::TRANSCRIPT_EXTRACT, |mut c: Caller<'_, StoreData>, path_ptr: i32, path_len: i32, offset: i64, harness_ptr: i32, harness_len: i32| -> i64 {
        ret_i64(with_mem(&mut c, |mem| {
            dispatch::transcript_extract_async(mem, path_ptr, path_len, offset, harness_ptr, harness_len)
        }))
    })?;

    linker.func_wrap(m, im::FS_LIST, |mut c: Caller<'_, StoreData>, path_ptr: i32, path_len: i32, flags: i32, out_ptr: i32, out_cap: i32| -> i64 {
        ret_i64(with_mem(&mut c, |mem| {
            dispatch::fs_list_async(mem, path_ptr, path_len, flags, out_ptr, out_cap)
        }))
    })?;
    ...
    linker.func_wrap(m, im::HOME_DIR, |mut c: Caller<'_, StoreData>, out: i32, cap: i32, len_out: i32| -> i32 {
        ret_i32(with_mem(&mut c, |mem| dispatch::home_dir(mem, out, cap, len_out)))
    })?;
