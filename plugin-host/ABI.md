...
copy on that server), `E_DENIED`(13, the peer may not call this method
here: a link this side made may call only a method the callee declared
`[caps.services] serve_remote` and the user allowed with `plugin-peers`;
an inbound peer's calls are always allowed).
