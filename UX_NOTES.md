...
- **What the main thread pays per read: decoding, not parsing.** The
  scan and the parse both run on the fs worker (`transcript_extract`,
  with the harness formats in `host/src/transcript.rs`); the guest
  receives finished turns, a few KB per call, and writes them to the
  store. A turn-end read is one call. The 172 MB transcript reads end
  to end in ~235 ms wall, 1,868 turns, none of it on the main thread
  beyond the block decode and the store batch. Rows from before the
  transcript existed get their file found by session id at start
  (`transcript::backfill`) and read once. `tools/dev-server.sh` runs it
